...
export default function AssignPoolsPage({ params }) {
  ...
  const fetchEventDetails = async () => {
    ...
    try {
      ...
      if (
        eventData.poolsAssigned &&
        eventData.pools &&
        eventData.pools.length > 0
      ) {
        ...
        const initialPoolConfig = eventData.trainers.map((trainer, index) => ({
          ...
          capacity: Math.ceil(
            eventData.registrations.length / eventData.trainers.length,
          ),
          meetLink: '',
          attendees: [],
        }))
  ...
  const handleMeetLinkChange = (poolId, meetLink) => {
    setPoolConfig(pools =>
      pools.map(pool => (pool.id === poolId ? { ...pool, meetLink } : pool)),
    )
  }
  ...
  const distributeUsers = () => {
    ...
    const totalCapacity = poolConfig.reduce(
    ...
    if (totalCapacity < users.length) {
      alert(
        `Warning: Total pool capacity (${totalCapacity}) is less than the number of registered users (${users.length})`,
      )
    }
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (missingLinks) {
        if (
          !confirm(
            'Some pools are missing meet links. Do you want to continue anyway?',
          )
        ) {
          setSubmitting(false)
          return
              ...
              >
                <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-4'>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => {
                        const selectedTrainer = event.trainers.find(
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={pool.capacity}
                      onChange={e =>
                        handlePoolCapacityChange(pool.id, e.target.value)
                      }
                      className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500'
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={pool.meetLink}
                      onChange={e =>
                        handleMeetLinkChange(pool.id, e.target.value)
                      }
                      className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500'
                ...
                <div className='mt-3'>
                  <h4 className='text-sm font-medium mb-2'>
                    Attendees ({pool.attendees ? pool.attendees.length : 0}/
                    {pool.capacity})
                  </h4>
                  {pool.attendees && pool.attendees.length > 0 ? (
                    ...
                    <div className='text-sm text-gray-500 italic'>
                      No attendees assigned. Use "Auto-Distribute Users" to
                      assign users.
                    </div>
                  )}
                </div>
